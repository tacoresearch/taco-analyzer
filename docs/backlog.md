...
**Art: JAREN.** A logo exists (isometric microscope in mint/jade green, with a
taco on the stage). It has not been committed to the repo yet.

### Live bug to fix alongside it

`server/views/layout.js` emits `<link rel="icon" href="/favicon.svg">` and
`server/routes/assets.routes.js` maps `/favicon.svg` and `/favicon.ico`, but
**`public/favicon.svg` does not exist**, so every page load 404s on the favicon.
Harmless, but it is noise in the logs and a missing icon in every browser tab.

### What is needed from JAREN

1. **Transparent background.** The version supplied has white behind it, which
   will render as a white box against the dark theme.
2. **SVG for preference.** Dropping the file at `public/favicon.svg` needs no
   code change at all: the route and the link element already point there. A PNG
   works but needs a new entry in the asset allowlist, which is an explicit map
   rather than a directory server, so files are never auto-served.
3. **A simplified square mark for the favicon.** The full logo is roughly 2:3
   portrait, and a detailed isometric microscope squashed into 16x16 is mush.
   Something like the objective lens over the taco, or the taco alone.
4. **A 512x512 PNG for home-screen icons.** This app is phone-first, and adding
   it to an Android home screen without one gets a generated letter tile.

### Wiring, once the art lands

- Favicon: drop in `public/`, add to the `ASSETS` map if it is not `favicon.svg`.
- Header: next to `.wordmark` in `layout.js`. Needs a size that does not crowd
  the nav at 360px; the header already wraps, so check it there first.
- Login page: above the sign-in card, where there is room for the full artwork.
- Home screen: a `manifest.webmanifest` (also needs an `ASSETS` entry and a
  `<link rel="manifest">`), with `name`, `short_name`, `theme_color`, and the
  icon sizes.

No CSP change is required: `img-src 'self'` already covers a same-origin file.
Do not inline the logo as a `<style>` background, which `style-src 'self'` would
block; use an `<img>` or inline `<svg>` element.

### While you are in there

The wordmark green (`--accent-baja-text`, `#2AA97A` dark and `#0E7A52` light) was
matched to a supplied colour sample and is close to the logo's mint. Once the real
artwork is in place, check the two against each other in both themes and settle
on one value, rather than leaving a wordmark and a logo that are almost the same
green.
